...
class PlaceInfo {
  ...
  PlaceInfo({this.rating, this.address, this.review, this.reviews});
}
...
class PlacesService {
  static Future<PlaceInfo?> getPlaceInfo(String name, String city) async {
    ...
    if (searchData['candidates'] == null || searchData['candidates'].isEmpty)
      return null;
    final placeId = searchData['candidates'][0]['place_id'];
    // 2. Obtener detalles del lugar (reviews en español)
    final detailsUrl = Uri.parse(
      'https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeId&fields=rating,formatted_address,reviews&language=es&key=$apiKey',
    );
    ...
    final rating =
        result['rating'] != null ? (result['rating'] as num).toDouble() : null;
    final address = result['formatted_address'] as String?;
    String? review;
    List<String> reviews = [];
    if (result['reviews'] != null && result['reviews'].isNotEmpty) {
      // Filtrar reviews en español
      for (var r in result['reviews']) {
        if (r['language'] == 'es') {
          reviews.add(r['text']);
        }
      }
      if (reviews.isNotEmpty) {
        ...
        // Si no hay en español, usar la primera
